...
class Sketchpad {
 public:
  ...
  Sketchpad(const glm::vec2& top_left_corner,
            ...
            double brush_radius = 1.15 /* default set */);
  ...
  /**
   * Perform Naive Bayes classification from the generated sketch image.
   * @return classified class from the class with highest likelihood.
   */
  size_t GetClassifiedClass();


 private:
  /** Th 2d int vector translated from sketched image on sketchpad. */
  std::vector<std::vector<int>> drawn_image;
